...
#pragma region Public Editor Methods
	...
	void set_maximum_culling_distance(real_t distance);

	real_t get_maximum_culling_distance() const;

	void set_collision_range(real_t range);

	real_t get_collision_range() const;
...
private:
	...
	real_t m_maximumCullingDistance = 1000.0; // 1km default for first-person

	real_t m_collisionRange = 100.0; // Only load collisions within 100m
protected:
